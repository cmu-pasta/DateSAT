"""
Hybrid DateSAT implementation using dual representation — both-initial variant.

This module implements a hybrid approach where dates are represented by BOTH
(Y, M, D) and epoch days from the moment a user variable is created: the Y/M/D
components and the epoch counter are materialized eagerly and linked once via
the encode formula epoch == days_since_epoch_from_ymd(y, m, d). Both
representations are therefore consistent from the start, and every operation
dispatches to whichever side is cheaper:

- DateVar vs DateVar comparisons: epoch (a single integer atom, instead of a
  lexicographic Or/And tree over six variables)
- DateVar vs concrete Date comparisons: Y/M/D lexicographic against constants
  (gives the solver direct interval information on the year variable without
  reasoning through the encode bridge)
- Field access (.year/.month/.day): Y/M/D directly
- Date + Period with month/year components: Y/M/D arithmetic (normalize, EOM clamp)
- Date + Period with only days: epoch arithmetic (single addition)

Intermediate results of arithmetic stay lazy (single representation installed
by __add__), identical to the hybrid_ymd variant — eagerly materializing both
sides for every intermediate would add one encode/decode div-mod chain per
arithmetic step for no benefit.
"""
...
class DateVar:
    """Symbolic date variable with eager dual representation (epoch + Y/M/D)."""
    ...
    def __init__(self, ctx, name: str, is_user_var: bool = True):
        ...
        Args:
            ...
            is_user_var: If True, this is a user-declared variable. User vars are
                eagerly materialized with BOTH representations consistent (Y/M/D
                vars plus epoch var linked by the encode formula). Intermediate
                results created by __add__ leave the YMD side unmaterialized and
                let __add__ install whichever representation the operation produced.
        """
        self.ctx = ctx
        self.name = name
        ...
        if is_user_var:
            # Both-initial variant: materialize Y/M/D eagerly AND treat the linked
            # epoch var as consistent, so operations can pick the cheaper side.
            self._year_var = Int(f"{name}_year")
            self._month_var = Int(f"{name}_month")
            self._day_var = Int(f"{name}_day")
            self._ymd_exists = True
            # Link epoch_var to Y/M/D via the encode formula (asserted exactly once)
    ...
    def __ge__(self, other) -> BoolRef:
        ...
        Comparison strategy (both-initial):
        1. Against a concrete Date with consistent Y/M/D: lexicographic against
           constants (direct interval information on the year variable)
        2. DateVar vs DateVar with both epochs consistent: single epoch atom
        3. Fallbacks mirror the lazy variants
        """
        if isinstance(other, Date):
            ...
            # Case 1: Both have consistent epoch - use epoch comparison (single atom)
    ...
    def __le__(self, other) -> BoolRef:
        """Support x <= date comparison (same strategy as __ge__)."""
        if isinstance(other, Date):
            # If we have consistent Y/M/D, use Y/M/D comparison (more efficient)
            if self._ymd_consistent and self._ymd_exists:
                y1, m1, d1 = self._year_var, self._month_var, self._day_var
    ...
    def __eq__(self, other) -> BoolRef:
        ...
        if isinstance(other, Date):
            ...
            # Case 3: Mixed states - derive the missing epoch and compare on epoch
            self._epoch_expr()
            other._epoch_expr()
            return self.epoch_var == other.epoch_var
        else:
    ...
    def _add_bounds(self) -> None:
        ...
        # Well-formedness: when Y/M/D vars exist as free Z3 integers, the encode
        # bridge (epoch = days_since_epoch_from_ymd(y,m,d)) doesn't force
        # well-formedness on Y/M/D - the solver could pick m=17, d=100. So we
        # constrain m in [1,12] and d in [1, days_in_month(y,m)] here.
        # The range bound (if any) follows the configured bound spec and is
        # applied on the Y/M/D side (the bridge then bounds the epoch var
        # implicitly); epoch-only intermediates get the equivalent day interval:
        ...
        if self._ymd_consistent and self._ymd_exists:
            ...
            # Epoch-only intermediate: bound epoch_var to the equivalent day interval.
    ...
    def __add__(self, other) -> 'DateVar':
        ...
        if isinstance(other, Period):
            ...
            if other.years == 0 and other.months == 0:
                # Direct assignment - epoch side is already consistent for user vars,
                # so this is a single addition with no conversion
                ...
                if od == IntVal(0):
                    ...
                    # Link epoch_var to the Y/M/D values so the result also has a
                    # consistent epoch side - comparisons against it stay single-atom
...
class HybridBothSolver:
    """Hybrid date constraint solver using eager dual representation.

    Fresh DateVars materialize BOTH representations upfront: Y/M/D vars with
    well-formedness bounds, an epoch var, and the encode bridge linking them.
    Every operation then dispatches to the cheaper side - epoch for DateVar
    comparisons and day arithmetic, Y/M/D for constant comparisons, field
    access, and month/year arithmetic - without ever asserting another
    conversion for user variables.
    """
