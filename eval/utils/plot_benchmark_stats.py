...
def main():
    ...
    arg_parser.add_argument(
        "datesatbench_dir",
        type=str,
        help="Path to the datesatbench directory containing llm_constraints/, "
        "grammar_constraints/, and legal_doc_constraints/",
    )
    arg_parser.add_argument(
        "--output",
        "-o",
        ...
        help="Output file path for the figure (default: <datesatbench_dir>/results/benchmark_stats.pdf)",
    )
    arg_parser.add_argument(
        "--atoms-log",
        type=str,
        default="atoms.log",
        help="Output file path for the atoms log (default: <datesatbench_dir>/results/atoms.log)",
    )
    arg_parser.add_argument(
        "--figsize",
        type=float,
    ...
    base_path = Path(args.datesatbench_dir).expanduser().resolve()
    if not base_path.is_dir():
        print(f"Error: Dataset directory '{base_path}' does not exist.")
        return 1

    # Create results folder inside the datesatbench directory
