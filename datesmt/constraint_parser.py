...
class ConstraintTransformer(Transformer):
    ...
    def _comparison_helper(self, items) -> str:
        ...
        transformed = self._transform_parametric_date_comparison(left_str, op_str, right_str)
        if transformed:
            return transformed
    ...
    def _check_variable_type(self, var_name: str, expected_type: str, context: str) -> None:
        """
...
class ConstraintParser:
    ...
    def parse_constraint(self, constraint_str: str, variable_types: Dict[str, str] = None) -> str:
        """
        ...
        try:
            ...
            if variable_types or self.variable_types:
                self._validate_parse_tree_types(tree, variable_types or self.variable_types)
    ...
    def infer_variable_types_from_context(self, constraints: List[str], skip_declared: Dict[str, str] = None) -> Dict[str, str]:
        """
    ...
    def _infer_types_from_tree(self, tree, inferred_types: Dict[str, str], skip_declared: Dict[str, str] = None) -> None:
        """
        ...
        if rule == 'int_comparison':
            ...
            has_bool_literal = any(
                isinstance(child, Tree) and (
                    (child.data == 'variable' and str(child.children[0]) in ['True', 'False']) or
                    child.data == 'bool_literal'
                )
                for child in tree.children
            )
        ...
        elif rule in ['int_expr', 'int_term', 'int_atom', 'int_add', 'int_sub', 'int_mul', 'int_neg']:
            for child in tree.children:
                if isinstance(child, Tree) and child.data == 'variable':
                    var_name = str(child.children[0])
                    # Skip True/False as they're boolean literals
        ...
        for constraint in constraints:
            ...
            for match in date_matches:
                ...
                for arg, component_type in zip(args, component_types):
                    ...
                    if re.search(r'\b[a-zA-Z_][a-zA-Z0-9_]*\s*\.\s*(?:year|month|day)\b', arg):
                        continue
                    
                    # Check if the expression contains any keywords that should be excluded
                    expr_vars = re.findall(r'\b[a-zA-Z_][a-zA-Z0-9_]*\b', arg)
                    if any(v in ['Date', 'Period', 'And', 'Or', 'Not', 'Implies', 'and', 'or', 'not', 'True', 'False', 'year', 'month', 'day'] for v in expr_vars):
                        continue
    ...
    def generate_builder_code(
        ...
        inferred_types = self.infer_variable_types_from_context(filtered_constraints, skip_declared=variable_types)
        ...
        if undeclared_vars:
            ...
            still_undeclared = [var for var in undeclared_vars if var not in variable_types]
            if still_undeclared:
