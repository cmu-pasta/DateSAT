...
verus! {
    ...
    impl DateEncoding for Hybrid {
        open spec fn from_ymd(y: int, m: int, d: int) -> Hybrid {
            // Epoch-first variant: literals embed via from_epoch_delta, so the
            // result carries only the computed epoch delta and the ymd side is
            // materialized lazily on first use.
            Hybrid::from_epoch_delta(EpochDelta::from_simple_date(SimpleDate(y, m, d)))
        }
    ...
    // In the epoch-first variant the embedding goes through from_epoch_delta,
    // so we delegate to the corresponding congruence theorem after
    // re-expressing SimpleDate(y, m, d) as the round-trip of its epoch delta.
    ...
    {
        let ed = EpochDelta::from_simple_date(SimpleDate(y, m, d));
        theorem_epoch_delta_to_ymd_from_simple_date_inverse(SimpleDate(y, m, d));
        theorem_hybrid_from_epoch_delta_congruent(ed);
    }
