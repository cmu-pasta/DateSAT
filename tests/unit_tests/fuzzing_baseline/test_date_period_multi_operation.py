...
@pytest.mark.fuzzing
def test_stepwise_multi_op_fuzzing_matches_python(base: Date, seq: list[Period]):
    expected = _apply_sequence_python(base, seq)
    res = _solve_with_solver(HypothesisSolver, base, seq)
    assert res["status"] == "sat", f"Expected sat for {base} + {seq}, got {res['status']}"
    assert res["dates"]["y"] == expected


@pytest.mark.parametrize("base,seq", MULTI_OP_CASES)
@pytest.mark.fuzzing
def test_stepwise_multi_op_sub_fuzzing_matches_python(base: Date, seq: list[Period]):
    res = _solve_with_solver_sub(HypothesisSolver, base, seq)
    if res["status"] == "unsat":
        return
    expected = _apply_sequence_python(base, seq)
    assert res["dates"]["y"] == expected
