...
@pytest.mark.fuzzing
def test_fuzzing_date_comparisons_match_truth(op_name: str, op, a: Date, b: Date):
    expect = _expect_truth(op, a, b)
    sat = _solve_compare(HypothesisSolver, a, b, op_name)
    assert sat == expect, f"Fuzzing: expected {expect} for {a} {op_name} {b}"
