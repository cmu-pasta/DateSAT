...
def test_stepwise_two_months_from_jan_30_non_leap():
    ...
    # Baseline
    s = BaselineSolver()
    x = s.add_date_var("x"); s.add_constraint(x == base)
    t1 = s.add_date_var("t1"); s.add_constraint(t1 == x + p1)
    t2 = s.add_date_var("t2"); s.add_constraint(t2 == t1 + p1)
    y = s.add_date_var("y"); s.add_constraint(y == t2)
    res = s.solve(); assert res["status"] == "sat"
    assert res["dates"]["y"] == Date(2023, 3, 28)

    # Advanced
    s2 = AdvancedDateSolver()
    x = s2.add_date_var("x"); s2.add_constraint(x == base)
    t1 = s2.add_date_var("t1"); s2.add_constraint(t1 == x + p1)
    t2 = s2.add_date_var("t2"); s2.add_constraint(t2 == t1 + p1)
    y = s2.add_date_var("y"); s2.add_constraint(y == t2)
    res2 = s2.solve(); assert res2["status"] == "sat"
