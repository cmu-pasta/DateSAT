...
@pytest.mark.parametrize(
    "base,per,label,seq",
    [
        pytest.param(base, per, label, seq, id=f"enumeration_{base}+{per}_{label}")
        for base, per, label, seq in all_decomposed_cases()
    ],
)
@pytest.mark.enumeration
def test_enumeration_matches_java_decomposed(
    base: Date, per: Period, label: str, seq: list[Period]
):
    ...
    assert (
        got == expect
    ), f"Enumeration order {label}: {base} + {per} -> {got}, expected {expect}"


@pytest.mark.parametrize(
    "base,per,label,seq",
    [
        pytest.param(base, per, label, seq, id=f"enumeration_sub_{base}+{per}_{label}")
        for base, per, label, seq in all_decomposed_cases()
    ],
)
@pytest.mark.enumeration
def test_enumeration_sub_matches_java_decomposed(
    base: Date, per: Period, label: str, seq: list[Period]
):
    ...
    assert (
        got == expect
    ), f"Enumeration sub {label}: {base} + {per} -> {got}, expected {expect}"
