...
def java_localdate_plus(year: int, month: int, day: int, y: int, m: int, d: int) -> Date:
...
@given(
    st.integers(min_value=1900, max_value=2099),  # keep within both libs' safe range
    st.integers(min_value=1, max_value=12),
    st.integers(min_value=1, max_value=28),  # conservative day to avoid invalid dates
    st.integers(min_value=-50, max_value=50),  # years
    st.integers(min_value=-120, max_value=120),  # months
    st.integers(min_value=-400, max_value=400),  # days
)
def test_java_python_period_plus_equivalence(base_year, base_month, base_day, py, pm, pd):
    ...
    # Ensure result is inside the domain supported by our Date (1900..2100)
    assume(1900 <= py_result.year <= 2100)
    ...
    assert (
        java_result == Date.from_python_date(py_result)
