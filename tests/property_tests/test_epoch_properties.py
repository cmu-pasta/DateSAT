...
class TestEpochConversionProperties:
    ...
    def test_epoch_conversion(self, d):
        ...
        epoch_days = days_since_epoch_from_ymd(custom_date)
        reconstructed = ymd_from_days_since_epoch(epoch_days)
    ...
    def test_epoch_conversion_inverse(self, days):
        ...
        date_obj = ymd_from_days_since_epoch(days)
        reconstructed_days = days_since_epoch_from_ymd(date_obj)
    ...
    def test_epoch_conversion_monotonic(self, d):
        ...
        next_epoch = days_since_epoch_from_ymd(next_custom)
    ...
    def test_epoch_conversion_consistency(self, d):
        ...
        custom_date = Date.from_python_date(d)
        epoch1 = days_since_epoch_from_ymd(custom_date)
        epoch2 = days_since_epoch_from_ymd(custom_date)
    ...
    def test_epoch_arithmetic_properties(self, days_offset):
        ...
        base_epoch = days_since_epoch_from_ymd(base_date)

        # Add days and verify
        target_date = ymd_from_days_since_epoch(base_epoch + days_offset)
        target_epoch = days_since_epoch_from_ymd(target_date)
